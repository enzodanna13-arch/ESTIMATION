...
const FINAL_SCHEMA = {
  ...
  properties: {
    ...
    positionnement_marche: { type: "string", description: "Synthèse : justifie les DEUX bornes de la fourchette en citant la médiane DVF (base de calcul), la médiane concurrentielle (positionnement) et le choix de rester sous cette dernière" },
...
- MÉTHODE D'ESTIMATION (chemin imposé, dans cet ordre) :
  ...
  2. ANALYSE CONCURRENTIELLE → médiane concurrentielle = niveau de prix des biens équivalents actuellement affichés (étude de marché) : elle sert au POSITIONNEMENT, jamais de base de calcul.
  3. CALCUL → fourchette de commercialisation = base_mediane + plus-values (spécificités du bien) − décotes (conjoncture, défauts).
  4. AFFINAGE CONCURRENTIEL → ajuste la fourchette finale pour un positionnement optimal : le prix de présentation (prix_presentation, scénario « Prix optimal ») doit ressortir SOUS la médiane concurrentielle des biens équivalents dès que les données le permettent — le bien doit apparaître comme le meilleur rapport qualité/prix de sa catégorie. Si le calcul de l'étape 3 ressort au-dessus, resserre la fourchette vers le bas et justifie-le.
- references_dvf : sélectionne 4 à 6 ventes réelles dans la liste DVF fournie (reprends celles de l'étude de marché si elle en contient), en PRIVILÉGIANT les surfaces proches du bien (±25 %) pour que la médiane soit directement représentative — dès que la liste DVF n'est pas vide, references_dvf ne doit JAMAIS être vide. base_mediane = la médiane des PRIX ACTÉS de ces références, telle quelle (le chiffre affiché sous le tableau des comparables du dossier) — ne la transpose NI au m² NI à la surface du bien. Rédige analyse_dvf.
- ajustements : pars de base_mediane (l'ancre) et liste les PLUS-VALUES (montants positifs : atouts réels du bien — extérieur, DPE, état issu des photos, stationnement, annexes…) et les DÉCOTES (montants négatifs : défauts réels — nuisances, travaux, conjoncture…) dont la somme aboutit exactement à prix_estime. Chaque ligne est une caractéristique concrète du bien ou du marché, JAMAIS une correction technique abstraite. Si la surface du bien diffère sensiblement de celle des références, exprime-le comme une seule ligne « Surface supérieure/inférieure aux références (X m² vs Y m² médians) » — c'est une caractéristique du bien comme une autre. Inclus OBLIGATOIREMENT une décote « Conjoncture — marché baissier (taux, pouvoir d'achat) ».
